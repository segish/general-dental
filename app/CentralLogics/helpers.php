...
class Helpers
{
    ...
    public static function send_push_notif_to_group($fcm_tokens, $data)
    {
        $key = BusinessSetting::where(['key' => 'push_notification_key'])->first()->value;
    ...
    public static function tax_calculate2($product, $price)
    {
        $discount = self::discount_calculate($product, $price);
        $price_tax = ($price - $discount) / 100 * $product['tax'];
        return $price_tax;
    }
    ...
    // public static function upload(string $dir, string $format, $image)
    // {
    //     $imageName = date('Y-m-d') . '-' . uniqid() . '.' . $format;
    //     Storage::disk('public')->put($dir, $imageName, file_get_contents($image));

    //     return str_replace('assets/', '', $dir) . $imageName;
    // }
    ...
    //     // Return the relative path like: assets/avatar/filename.jpg
    ...
    //     $avatarDir = 'assets/' . trim($dir, '/') . '/'; // store under assets/avatar/

    //     Storage::disk('public')->put($avatarDir . $imageName, file_get_contents($image));

    //     // Return only avatar/filename.png to save in DB
    //     return trim($dir, '/') . '/' . $imageName;
    // }
    ...
    public static function upload(string $dir, string $format, $image)
    {
        ...
        // Storage path
        $storageDir = 'assets/' . trim($dir, '/') . '/';
        $publicPath = public_path('storage/' . $storageDir);
        $fullStoragePath = storage_path('app/public/' . $storageDir . $imageName);

        // Step 1: Save in storage (Laravel standard)
        Storage::disk('public')->put($storageDir . $imageName, file_get_contents($image));

        // Step 2: Make sure public folder exists
        if (!file_exists($publicPath)) {
            mkdir($publicPath, 0777, true);
        }

        // Step 3: Copy to public path (so /storage/assets/... works on cPanel)
        copy($fullStoragePath, $publicPath . $imageName);

        // Step 4: Return DB path like 'avatar/image.png'
        return trim($dir, '/') . '/' . $imageName;
    }
    ...
    public static function update(string $dir, $old_image, string $format, $image = null)
    {
        if ($old_image && Storage::disk('public')->exists('assets/' . $old_image)) {
            Storage::disk('public')->delete('assets/' . $old_image);
            $oldPublicPath = public_path('storage/assets/' . $old_image);
