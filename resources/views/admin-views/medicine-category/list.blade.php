...
@section('content')
                        ...
                        <div class="row gy-2 align-items-center">
                            ...
                            @if (auth('admin')->user()->can('patient.add-new'))
                                <div class="col-lg-8 col-sm-4 col-md-6 d-flex justify-content-sm-end">
                                    <a href="{{ route('admin.medicine_categories.add-new') }}" class="btn btn-primary">
                                        <i class="tio-add"></i>
                                        {{ \App\CentralLogics\translate('Add New Medicine Category') }}
                                    </a>
                                </div>
                            @endif
                        </div>
                                            ...
                                            <div class="d-flex gap-2 justify-content-center">
                                                ...
                                                @if (auth('admin')->user()->can('medicines.edit'))
                                                    <a class="btn btn-outline-primary square-btn"
                                                        href="{{ route('admin.medicine_categories.edit', [$category['id']]) }}">
                                                        <i class="tio tio-edit"></i>
                                                    </a>
                                                @endif
                                                @if (auth('admin')->user()->can('medicines.delete'))
                                                    <a class="btn btn-outline-danger square-btn" href="javascript:"
                                                        onclick="form_alert('patient-{{ $category['id'] }}','{{ \App\CentralLogics\translate('Want to delete this patient ?') }}')"><i
                                                            class="tio tio-delete"></i></a>
                                                @endif
                                            </div>
                                            <form
                                                action="{{ route('admin.medicine_categories.delete', [$category['id']]) }}"
                                                method="post" id="patient-{{ $category['id'] }}">
                                                @csrf @method('delete')
                                            </form>
                                        </td>
                                    </tr>
    ...
    <script>
        function deleteCategory(id) {
            if (confirm("{{ translate('Are you sure you want to delete this category?') }}")) {
