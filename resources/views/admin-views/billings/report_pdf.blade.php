...
<table border="1" cellpadding="5">
    <thead>
        <tr>
            ...
            <th>{{ \App\CentralLogics\translate('Recieved By') }}</th>
            <th>{{ \App\CentralLogics\translate('Tot_Amount') }}</th>
            <th>{{ \App\CentralLogics\translate('Rec_Amount') }}</th>
        ...
        @foreach ($billings as $key => $billing)
            <tr>
                ...
                <td>
                    ...
                    @if ($billing->amount_paid > 0 && $billing->amount_paid < $billing->total_amount)
                        <span style="font-weight: bold; color:rgba(255, 138, 14, 0.776)">Partial
                            <br> ({{ $billing->total_amount - $billing->amount_paid }}) </span>
                    @endif
                </td>
