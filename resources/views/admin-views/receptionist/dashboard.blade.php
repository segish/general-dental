    ...
    <div class="content container-fluid">
                ...
                <div class="row g-2" id="order_stats">
                    <div class="col-sm-6">
                        ...
                        <a class="dashboard--card" href="{{ route('admin.appointment.list') }}">
                            <h5 class="dashboard--card__subtitle">{{ \App\CentralLogics\translate('Todays Appointments') }}
                            </h5>
                    ...
                    <div class="col-sm-6">
                        <!-- Card -->
                        {{-- <a class="dashboard--card" href="{{route('admin.ward.list')}}">
                            <h5 class="dashboard--card__subtitle">{{\App\CentralLogics\translate('wards')}}</h5>
                            <h2 class="dashboard--card__title">{{$ward_count}}</h2>
                            <img width="30" src="{{asset(config('app.asset_path') . '/admin/img/icons/confirmed.png')}}" class="dashboard--card__img" alt="confirmed">
                        </a> --}}
                        <!-- End Card -->
                    </div>
                </div>
        ...
        {{-- <div class="table-responsive datatable-custom">
            <table class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table">
                ...
                <tbody id="set-rows">
                    ...
                    $appointmentsForToday = $appointments->filter(function($appointment) use ($today) {
                        $appointmentDate = date_create($appointment->date)->setTimezone(new DateTimeZone('Africa/Addis_Ababa'))->format('Y-m-d');
                    ...
                    @if (count($appointmentsForToday) > 0)
                        @foreach ($appointmentsForToday as $key => $appointment)
                            <tr>
                                ...
                                <td>{{ date_create($appointment->date)->setTimezone(new DateTimeZone('Africa/Addis_Ababa'))->format('M d, Y') }}</td>
                                <td>
            ...
            <div class="text-center p-4">
                <img class="mb-3" src="{{asset(config('app.asset_path') . '/admin')}}/svg/illustrations/sorry.svg" alt="Image Description" style="width: 7rem;">
