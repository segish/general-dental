                                    ...
                                    <div class="col-12">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="name" value="{{ $role->name }}"
                                                class="form-control" placeholder="{{ translate('Ex : Manager') }}" required>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="row">
                                                @foreach ($groupedPermissions as $group => $permissions)
                                                    <div class="col-xs-12 col-sm-12 col-md-12 pt-3 pb-3">
                                                        ...
                                                        <div id="{{ $group }}" class=" row collapse">
                                                            @foreach ($permissions as $value)
                                                                <div class="col-xs-6 col-sm-6 col-md-4">
                                                                    <label>
                                                                        {{ Form::checkbox('permission[]', $value->id, in_array($value->id, $userPermissions), ['class' => 'name', 'data-group' => $group]) }}
                                                                        {{ $value->name }}
                                                                    </label>
                                                                    <br />
                                                                </div>
...
@push('script')
    ...
    <script>
        $(document).ready(function() {
            // Check/uncheck all individual permissions
            $('.name').click(function() {
                var group = $(this).data('group');
                var isChecked = $(this).prop('checked');

                // Check/uncheck the entire group if all individual permissions are selected/unselected
                var allInGroupChecked = $('.name[data-group="' + group + '"]:checked').length === $(
                    '.name[data-group="' + group + '"]').length;
                $('input.check-all-group[data-group="' + group + '"]').prop('checked', allInGroupChecked);
            });

            // Check/uncheck all permissions in a group
            $('.check-all-group').click(function() {
                var group = $(this).data('group');
                var isChecked = $(this).prop('checked');

                // Check/uncheck all individual permissions in the group
                $('.name[data-group="' + group + '"]').prop('checked', isChecked);
            });
        });
    </script>
