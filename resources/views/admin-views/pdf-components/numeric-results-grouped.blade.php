<table>
    ...
    @php
        $groupedResults = $numericResults->groupBy(function ($result) {
            return $result->laboratoryRequestTest->test->testCategory->name;
        });
    @endphp

    @foreach ($groupedResults as $categoryName => $results)
        <tr>
            <td colspan="5" style="text-align: center; background-color: #f2f2f2;"><strong>{{ $categoryName }} Tests</strong></td>
        </tr>

        @foreach ($results as $result)
            @php
                $attributes = collect($result->attributes)->sortBy(function ($item) {
