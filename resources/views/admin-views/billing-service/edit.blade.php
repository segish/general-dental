...
@section('title', translate('update new service'))
    ...
    <div class="content container-fluid">
        <div class="mb-3">
            <h2 class="text-capitalize mb-0 d-flex align-items-center gap-2">
                <img width="20" src="{{ asset(config('app.asset_path') . '/admin/img/icons/product.png') }}"
                    alt="">
                {{ \App\CentralLogics\translate('update_service') }}
            </h2>
        </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('admin.service.update', $test->id) }}" method="post">
                            @csrf
                            @method('PUT')
                            <div class="row mb-3">
                                <div class="col-md-6 form-group">
                                    ...
                                    <input type="text" name="test_name" class="form-control"
                                        placeholder="{{ translate('New Test') }}" value="{{ $test->test_name }}" required>
                                </div>
                                <div class="col-md-6 form-group">
                                    <label class="input-label"
                                    ...
                                    <select name="test_category_id" class="form-control js-select2-custom" required>
                                        ...
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}"
                                                {{ $category->id == $test->test_category_id ? 'selected' : '' }}>
                                                {{ $category->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 form-group">
                                    ...
                                    <input type="number" name="cost" class="form-control"
                                        placeholder="{{ translate('Cost') }}" value="{{ $test->cost }}" required>
                                </div>
                                <div class="col-md-6 form-group">
                                    <label class="input-label"
                                    ...
                                    <input type="text" name="time_taken" class="form-control"
                                        placeholder="{{ translate('30 miniute') }}" value="{{ $test->time_taken }}"
                                        required>
                                </div>
                                <div class="col-12">
                                    <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <textarea name="description" class="ckeditor form-control">{{ $test->description }}</textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
